...
**해결: 이벤트 버퍼링.** 초기화 완료 전에 들어오는 이벤트를 배열에 담아두고, 초기화 끝나면 순서대로 flush한다.
...
Analytics 코드는 비즈니스 로직이 아니라 계측 코드다. 잘못 넣으면 티가 안 나고, 안 넣으면 데이터가 없다. "되는 것 같은데 실은 안 되고 있는" 상태가 가장 위험하다.
