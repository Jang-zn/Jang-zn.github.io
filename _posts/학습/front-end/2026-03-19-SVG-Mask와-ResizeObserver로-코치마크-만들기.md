...
`getBoundingClientRect()`는 호출 시점의 스냅샷만 준다. 변화를 추적하려면 `ResizeObserver`가 필요하다.
...
`{ passive: true }`는 "이 리스너는 `preventDefault()`를 호출하지 않겠다"는 약속이다. 브라우저가 이 약속을 믿고 리스너 실행을 기다리지 않고 스크롤을 바로 실행한다. 스크롤이 버벅이지 않는다.
