...
애초에 snake_case 설정을 넣은 것도 Claude고, 그걸 발견하고 "이거 왜 snake_case야, 빼자"라고 한 건 내 판단이다. Claude한테 실행을 맡겼는데, `JacksonConfig` 삭제 판단은 Claude가 자동설정 충돌을 분석해서 제안했다. "이 빈이 있으면 Spring Boot 자동설정이 꺼진다"는 분석이 정확했다. 범인이 현장 증거를 가장 잘 아는 법이다.
