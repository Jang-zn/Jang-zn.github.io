...
BE에서 `PropertyNamingStrategy.SNAKE_CASE`가 설정되어 있었다. 내가 한 게 아니다. 초기 세팅 때 Claude가 지맘대로 넣어둔 거다. 나는 서버도 FE도 다 만드는데, FE가 camelCase를 쓰는 건 당연하다. 서버가 snake_case를 내려보내니까 [Admin FE](/posts/API-클라이언트-인프라-구축/)에서 Axios 인터셉터로 변환하고, [TasteNote](/posts/약관과-Firebase-수정/)에서도 Raw 타입과 mapper에서 일일이 매핑하고 있었다. 프론트엔드 2곳에서 같은 변환을 중복으로 하고 있는 거다. 서버에서 camelCase로 내려주면 이 변환 로직이 전부 필요 없어진다.
...
애초에 snake_case 설정을 넣은 것도 Claude고, 그걸 발견하고 "이거 왜 snake_case야, 빼자"라고 한 건 내 판단이다. Claude한테 실행을 맡겼는데, `JacksonConfig` 삭제 판단은 Claude가 자동설정 충돌을 분석해서 제안했다. "이 빈이 있으면 Spring Boot 자동설정이 꺼진다"는 분석이 정확했다. 자기가 만든 문제를 자기가 잘 아는 셈이다.

Redis 캐시 충돌은 Claude가 먼저 잡았다. "네이밍 전략 바꾸면 기존 캐시 역직렬화가 실패합니다, 키 버저닝 하겠습니다"라고 선제적으로 처리. 이건 내가 놓칠 뻔한 부분이다.
