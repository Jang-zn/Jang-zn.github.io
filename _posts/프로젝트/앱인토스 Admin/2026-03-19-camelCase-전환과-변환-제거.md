...
어제 "API 연동 인프라 완성"이었는데, 오늘 그 인프라에서 불필요한 레이어를 하나 벗겨낸 거다. 서버가 클라이언트 친화적인 포맷을 내려주면 클라이언트 코드가 단순해진다. 우체국에서 편지를 받는 쪽 언어로 번역해서 보내주면 받는 사람이 번역기를 안 돌려도 되는 것과 같다. 당연한 얘기지만, 실제로 체감하니 좀 다르다.
